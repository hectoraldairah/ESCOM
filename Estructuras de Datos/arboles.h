...
void imprimirInwwder(NodoArbol *root)
{
    if(root)
    {
        imprimirInwwder(root->izquierda);
        printf("%d\n", root->info);
        imprimirInwwder(root->derecha);
    }
}
void imprimirPrewwder(NodoArbol *root)
{
    if(root)
    {
        printf("%d\n", root->info);
...
void imprimirPostwwder(NodoArbol *root)
{
    if(root)
    {
        imprimirInwwder(root->izquierda);
...
{
    if(!a && !b) // Comparaci�n de punteros: 0 = NULL
        return 1;
    else if(!a || !b)
        return 0;
    else if(a && b)
